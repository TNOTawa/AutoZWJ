...
将选中的多个模板物件按时间轴顺序视为一个音的动画序列：
- 以第一个物件的开始帧到最后一个物件的结束帧作为源范围
- 生成时把整个序列映射到音符经过同步模式计算后的统一范围
- 每个子物件保留相对于序列的时长和间隙
- 按模板物件的相对图层差输出，并以整段序列参与层分配
- “允许拉伸”开启时，根据同步策略拉伸相对时长；关闭时保留模板绝对时长，并从后半段开始裁切以适应音符范围

该功能仍处于 beta 阶段，部分模板链、层分配策略和复杂和弦组合尚未覆盖完整测试。

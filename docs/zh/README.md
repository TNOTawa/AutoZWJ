...
将 REAPER 工程文件（`.rpp`）、标准 MIDI 文件（`.mid`）、UTAU 工程文件（`.ust` / `.ustx`）或 LRC 歌词文件（`.lrc`）中的素材，以用户在 AviUtl2 中选定的物件为模板，批量生成到时间轴上。

## 功能概述

- 解析 REAPER `.rpp`、标准 MIDI `.mid`、UTAU `.ust`、OpenUTAU `.ustx`、LRC `.lrc` 等工程文件
- 以时间轴上已有物件为样式模板，继承其效果链、参数设定
- 多源映射 —— 选中多个模板物件，按策略（顺序轮替 / 随机抽选 / 和弦映射 / 动画序列 [beta]）分配
...
- 物件与音符同步：支持拉伸到下一音符、固定帧数、间隙生成，以及拉伸并固定最后一帧（可选延伸到下一音符）
...
2. **导入工程**<br>
   若尚未导入工程，插件窗口会自动进入工程导入页：将 `.rpp` / `.mid` / `.ust` / `.ustx` 文件拖入 AviUtl2 窗口，或在导入页选择最近文件 / 浏览...；随后勾选要生成的轨道，点击**确认导入**
...
- 使用的工程格式（RPP、MIDI、UST/USTX 或 LRC）及可复现的操作步骤
- 预期结果与实际结果
